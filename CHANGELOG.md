...
## Feature
- 基于原版v2.9.3版本重新开始
- 可以识别语音消息：通过接入`whisper-1`API实现
- 可以微调参数：通过设置可以节省token消耗量、让`ChatGpt`更准确地回答问题
- `ChatGpt Web`现在可以“记住”更多的上下文：服务器端将聊天记录被持久化保存
- 去掉了`ChatGPTUnofficialProxyAPI`和反向代理功能：考虑到使用者并非都有精力折腾，因此只保留了官方API的方式。
- 可以随机生成头像
- 增加日语界面
- 优化了一点移动端体验
- 后端使用python重写
